//const { default: MoviesDAO } = require("../dao/moviesDAO");
...
;(async () => {
  try {
    ...
    // xx     add a predicate that checks that the `lastupdated` field exists, and then
    // xx     check that its type is a string
    ...
    const moviesCollection = mflix.collection("movies")

    const { modifiedCount } = await moviesCollection.bulkWrite(
      moviesToMigrate,
      { ordered: false },
    )
